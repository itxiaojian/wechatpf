      ...
      <div class="top_01" style="overflow:auto;">
    	...
    	<div class="CXBiao">
          ...
          <select onChange="getWeek(this.value)">
            ...
            <option <c:if test="${week =='星期一'}"></c:if> value="星期一">星期一</option>
			<option <c:if test="${week =='星期二'}"></c:if> value="星期二">星期二</option>
			<option <c:if test="${week =='星期三'}"></c:if> value="星期三">星期三</option>
			<option <c:if test="${week =='星期四'}"></c:if> value="星期四">星期四</option>
			<option <c:if test="${week =='星期五'}"></c:if> value="星期五">星期五</option>
          </select>&nbsp;&nbsp;&nbsp;&nbsp;
          <select onChange="getDsWeek(this.value)">
            <option  value="">周次</option>
            <option <c:if test="${dsweek =='1'}"></c:if> value="1">1</option>
			<option <c:if test="${dsweek =='2'}"></c:if> value="2">2</option>
			<option <c:if test="${dsweek =='3'}"></c:if> value="3">3</option>
			<option <c:if test="${dsweek =='4'}"></c:if> value="4">4</option>
			<option <c:if test="${dsweek =='5'}"></c:if> value="5">5</option>
			<option <c:if test="${dsweek =='6'}"></c:if> value="6">6</option>
			<option <c:if test="${dsweek =='7'}"></c:if> value="7">7</option>
			<option <c:if test="${dsweek =='8'}"></c:if> value="8">8</option>
			<option <c:if test="${dsweek =='9'}"></c:if> value="9">9</option>
			<option <c:if test="${dsweek =='10'}"></c:if> value="10">10</option>
			<option <c:if test="${dsweek =='11'}"></c:if> value="11">11</option>
			<option <c:if test="${dsweek =='12'}"></c:if> value="12">12</option>
			<option <c:if test="${dsweek =='13'}"></c:if> value="13">13</option>
			<option <c:if test="${dsweek =='14'}"></c:if> value="14">14</option>
			<option <c:if test="${dsweek =='15'}"></c:if> value="15">15</option>
			<option <c:if test="${dsweek =='16'}"></c:if> value="16">16</option>
			<option <c:if test="${dsweek =='17'}"></c:if> value="17">17</option>
			<option <c:if test="${dsweek =='18'}"></c:if> value="18">18</option>
			<option <c:if test="${dsweek =='19'}"></c:if> value="19">19</option>
			<option <c:if test="${dsweek =='20'}"></c:if> value="20">20</option>
          </select>&nbsp;&nbsp;&nbsp;&nbsp;
    ...
    <div class="main_02" style="top:240px;">
    	<div class="M_box">
        	<div class="M_msg">
                ...
                <div class="M_main" style="width:100%;">
                <c:forEach var="data" items="${list}" varStatus="obj">
                	<ul style="width:100%;">
                    	<div class="SKDi" style="width:100%;">
                        	${data.jxl}
                        </div>
                    	<li>
                        	<span class="msg_main01" style="text-align:center;text-indent:0px;">教室名称</span>
                            <span class="msg_main02" style="text-align:center;text-indent:0px;">上课情况</span>
                        </li>
                        <c:forEach var="dataJs" items="${data.listJS}" varStatus="obj"> 
                        <li>
                        	<span class="msg_main001" style=" text-align:center;">${dataJs.name}</span>
                            <span class="msg_main002">  <c:choose>
						   			<c:when test="${dataJs.jsqk =='0'}">
						    			空
						  		 	</c:when>
						   			<c:otherwise>
						   	 				 ${dataJs.jsqk}
