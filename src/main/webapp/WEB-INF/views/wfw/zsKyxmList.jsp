...
function loadMore(page,openId,qh){
	i=page;
	i++;
	var url ="<%=path%>/wfw/ZsKyxm/toKyxmZj";
	var html= "";
    $.ajax({
		url :url,
		data : {
		...
		success : function(data) {
		    ...
		    html+="<div id='lidw"+i+"' class='maring1'>"
				...
				+"<tr class='odd-row'>"
			    +"<td class='first'>项目名称</td>"
			    +"<td class='last'>"+value.xmmc+"</td>"
			    +"</tr>"
			    +"<tr class=''>"
			    ...
			    +"</tr>"
			    +"<tr class=''>"
			    +"<td class='first'>资助经费信息</td>"
			    +"<td class='last'>"+value.zzjf+"</td>"
			    +"</tr>"
			    +"<tr class='odd-row'>"
			    +"<td class='first'>立项日期</td>"
			    +"<td class='last'>"+value.lxrq+"</td>"
			    +"</tr>"
			    +"<tr class=''>"
...
<div class="top_01">
		...
		<form action="<%=path%>/wfw/ZsKyxm/toKyxm" id="myForm" class="wwx_f_a"  method="post" style="display: inline;">
