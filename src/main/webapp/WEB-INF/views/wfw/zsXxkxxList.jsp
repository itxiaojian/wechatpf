...
// 		var objS = document.getElementById("pid");
// 	    var grade = objS.options[objS.selectedIndex].value;
<%-- 	    location.href ="<%=path%>/wfw/ZsXxkxx/toXxkxxByQh?pages="+i+"&openId="+openId+"&ksqh="+grade; --%>
// 	}
	...
	function loadMore(pages,openId,qh){
		...
		i++;
		var size = (i-1)*10;
		var url ="<%=path%>/wfw/ZsXxkxx/xxkjzgd";
		var html= "";
	    $.ajax({
			url :url,
			data : {
			...
			success : function(data) {
			    ...
			    html += "<ul><li><span class='msg_main01'>课程名称</span>"
			         +"<span class='msg_main02' style='color:#fc4312'>"
			         +Null(value.kcmc)
			    	 +"</span></li>"
			    	 +"<li><span class='msg_main01'>课程编号</span>"
				     +"<span class='msg_main02'>"
	               	 +Null(value.kcbh)
	                 +"</span></li>"
				     +"<li><span class='msg_main01'>上课地点</span>"
				     +"<span class='msg_main02'>"
	               	 +Null(value.skdd)
	                 +"</span></li>"
	                 +"<li><span class='msg_main01'>任课老师</span>"
	                 +"<span class='msg_main02'>"
	                 +Null(value.lsmc)
	                 +"</span></li>"
	                 +"<li><span class='msg_main01'>备注</span>"
	                 +"<span class='msg_main02'>"
	                 +Null(value.bz)
	                 +"</span></li></ul>";
				})
...
.M_main ul{ float:left; padding-bottom:20px;border-bottom: 1px solid #8bc0f2;}
	...
	<div class="M_box">
	    <div class="M_msg">
	     <c:if test="${!empty cjlist}">
	         ...
	         <c:if test="${data1.jsmc=='ROLE_STUDENT' }">
		         <div class="M_title">
		             <img style="margin-top:3%; margin-left:2%;width:10%" src="<%=path%>/resources/img/wzy/tsjy.png" >${data1.xsxm}
		             <span>&nbsp;&nbsp;学号:${data1.xh}</span>
		               <img style="float:right; margin-top:3%; margin-right:2%;width:10%" src="<%=path%>/resources/img/wzy/tsdian.png">
		         </div>
		      </c:if>
		       <c:if test="${data1.jsmc=='ROLE_TEACHER' }">
		         <div class="M_title">
		             <img style="margin-top:3%; margin-left:2%;width:10%" src="<%=path%>/resources/img/wzy/tsjy.png" >${data1.lsmc}
		             <span>&nbsp;&nbsp;工号:${data1.sklsgh}</span>
		               <img style="float:right; margin-top:3%; margin-right:2%;width:10%" src="<%=path%>/resources/img/wzy/tsdian.png">
		         </div>
		         </c:if>
		      </c:forEach>
	            <div class="M_main" id="TAB01${obj.count}" style="display: block;"> 
	             <c:forEach var="data" items="${cjlist}"> 
	                 <ul>
<%-- 	                <c:if test="${data1.xh == data.xh || data1.ksxn == data.xn || data1.ksqh == data.xq}"> --%>
		             
	                        <li>
	                        	<span class="msg_main01">课程名称</span>
	                            <span class="msg_main02" style="color:#fc4312">${data.kcmc}</span>
	                        </li>
	                        <li>
	                        	<span class="msg_main01">课程编号</span>
	                            <span class="msg_main02" >${data.kcbh}</span>
	                        </li>
	                        <li>
	                        	<span class="msg_main01">上课地点</span>
	                            <span class="msg_main02" >${data.skdd}</span>
	                        </li>
	                        
	                        <li>
	                        	<span class="msg_main01">任课老师</span>
	                            <span class="msg_main02" >${data.lsmc}</span>
	                        </li>
	                        <li>
