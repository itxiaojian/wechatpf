...
// 	i=page;
// 	i++;
<%-- 	location.href ="<%=path%>/wfw/ZsXjyd/toXjydZj?pages="+i+"&openId="+openId; --%>
// }
...
function loadMore(pages,openId,tj){
	...
	i++;
	var size = (i-1)*10;
	var url ="<%=path%>/wfw/ZsXjyd/xjjzgd";
	var html= "";
    $.ajax({
		url :url,
		data : {
		...
		success : function(data) {
		    ...
		    html += "<div class='M_title'><img style='margin-top:3%; margin-left:2%;width:10%;' src='<%=path%>/resources/img/wzy/tsjy.png'>"
                 ...
                 +"<span class='msg_main01'>异动类型</span>"
                 +"<span class='msg_main02' style='color:#fc4312'>"
                 +value.ydlx
                 +"</span></li><li>"
                 +"<span class='msg_main01'>详细情况</span>"
                 +"<span class='msg_main02'>"
                 +value.xxqk
                 +"</span></li><li>"
                 +"<span class='msg_main01'>发生时间</span>"
                 +"<span class='msg_main02'>"
                 +value.fssj
                 +"</span></li><li>"
...
.top_01 img{  display:block; float:right;margin-right:10%;} 
...
<div class="top_01" style="height:50px;">
		...
		<form action="<%=path%>/wfw/ZsXjyd/toXjyd" id="myForm" class="wwx_f_a"  method="post" style="display: inline;">
           ...
           <c:if test="${!empty list}">                                    
            ...
            <div class="M_main" id="TAB01${obj.count}" style="display: block;"> 
	             <ul>
	                <li>
	                  <span class="msg_main01">异动类型</span>
	                  <span class="msg_main02" style="color:#fc4312">${data.ydlx}</span>
	                </li>
	                 <li>
	                  <span class="msg_main01" >详细情况</span>
	                  <span class="msg_main02">${data.xxqk}</span>
	                </li>
	                <li>
	                  <span class="msg_main01">发生时间</span>
	                  <span class="msg_main02">${data.fssj}</span>
	                </li>
	                <li>
	                  <span class="msg_main01">备注</span>
