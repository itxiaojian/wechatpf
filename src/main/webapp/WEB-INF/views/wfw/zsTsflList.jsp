...
function loadMore(page,openId){
	i=page;
	i++;
	var url ="<%=path%>/wfw/ZsTsfl/toTsflZj";
	var html= "";
    $.ajax({
		url :url,
		data : {
		...
		success : function(data) {
		    ...
		    html+="<div id='lidw"+i+"' class='maring1'>"
			    ...
			    +"<td class='last'>"+value.zl+"</td>"
			    +"</tr>"
			    +"<tr class=''>"
			    +"<td class='first'>数量</td>"
			    +"<td class='last'>"+value.sl+"</td>"
			    +"</tr>"
			    +"<tr class='odd-row'>"
...
<div class="top_01">
		...
		<form action="<%=path%>/wfw/ZsTsfl/toTsfl" id="myForm" class="wwx_f_a"  method="post" style="display: inline;">
...
<!-- 			<p>图书分类信息暂无...</p> -->
   ...
   <c:otherwise>
        <div id="zjgd" class="-ft" style="margin-top: -20px;">
			<button class="btn btn-default btn-block btn-lg ng-binding" onclick="loadMore('${pages}','${openId}');">加载更多</button>
		</div>  
   </c:otherwise>  
</c:choose>
