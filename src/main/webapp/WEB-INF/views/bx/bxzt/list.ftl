...
<script type="text/javascript">
		...
		var zNodes =[
			<#list bxzt?if_exists as doc>
			{ id:'${doc.id}', <#if doc.sjzt?has_content>pId:"${doc.sjzt}",</#if> name:"${(doc.ztmc)!}",icon:"${base}/libs/images/icons/folder.gif",open:true}<#if doc_index != (bxzt?size -1 )>,</#if>
			</#list>
		];
		...
		function addHoverDom(treeId, treeNode) {
			...
			if (treeNode.id != "0"){
				...
				if (removeBtn) removeBtn.bind("click", function(){
					onAction({url:'${base}/bx/bxzt/delete?ids='+treeNode.id,callback:refresh,confirm:true});
					return false;
				});
				...
				if (editBtn) editBtn.bind("click", function(){
					onAction({url:'${base}/bx/bxzt/edit?id='+treeNode.id,callback:refresh});
					return false;
				});
			}
			...
			if (addBtn) addBtn.bind("click", function(){
				onAction({url:'${base}/bx/bxzt/add?pid='+treeNode.id,callback:refresh});
				return false;
			});
		};
...
<ul class="tab-menu tab" id="tabMenuID_">
	<li class="tab-selected" title="报修主题">
	<span>报修主题</span>
	</li>
</ul>
</div>
