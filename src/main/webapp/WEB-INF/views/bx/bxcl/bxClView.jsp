...
<script type="text/javascript">
 jQuery(function() {
	...
	window.location.href = "<%=path%>/bx/bxcl/cllist?bm=" + bm + "&bxzt="+bxzt+"&wxdl="+wxdl;
	});
...
<%-- 		art.dialog.open('<%=path%>/bx/bxcl/succ?id='+dm,{
			//id:'detailDialog',
			//title:'申请查看',
			//titleClass: 'dialog_diy',
			//content: document.getElementById('detail'),
...
<%-- 		art.dialog.open('<%=path%>/bx/bxcl/succ?id='+dm,{
	...
	function rejec(dm){
		art.dialog.open('<%=path%>/bx/bxcl/rejec?id='+dm,{
			//id:'detailDialog',
			//title:'申请查看',
			//titleClass: 'dialog_diy',
			//content: document.getElementById('detail'),
	...
	function rejecCheckBox(dm){
		art.dialog.open('<%=path%>/bx/bxcl/rejec?id='+dm,{
	        ...
	        ok:function(){
	        	var str = this.iframe.contentWindow.$('#areadropboxvalue').val();
	        	$.ajax({
					url:"<%=path%>/bx/bxcl/rejecCheckBox",
					type : 'get',
					//dataType : 'json',
					data:{
						idList:dm,
...
<body id="mainbody" style="overflow-y: scroll;" class="objbody1">
	<div style="display:none;">
		<ul class="tab-menu tab" id="tabMenuID_">
			<li class="tab-selected" title="报修处理">
				<a href="<%=path%>/bx/bxcl/cllist" target="content" onfocus="this.blur()"><span>报修处理</span></a>
			</li>
		</ul>
	</div>
	<div class="maindiv">
	<div class="tl pl10">
		...
		<div class="acbodydiv">
			<div>
					...
					<tbody>
						<tr>
						 ...
						 <tr>
							...
							<td style="width: 10%;">${data.sbztmc}</td>
							<td style="width: 8%;" class="nrtd${status.index}" title="${data.nr}">${data.nr}</td>
							...
							<td style="width: 6%;text-align:center;"> <a id="GridView1_ctl02_HyperLink4"
								onclick='javascript:see($(this).attr("alt"))'
								alt="${data.id}" href="#">查看</a>
								<a id="GridView1_ctl02_HyperLink4"
								onclick='javascript:succ($(this).attr("alt"))'
				...
				<div class="aui_button">
            <input id="Button3" class="button aui_state_highlight" value="回访" type="button">&nbsp;
	...
	<c:choose>
					<c:when test="${pages > 1}">
						<a href="<%=path%>/bx/bxsh/cllist?pages=${pages - 1}">上一页</a>
					</c:when>
					<c:otherwise>
						上一页
					</c:otherwise>
				...
				<c:choose>
					<c:when test="${pages < count}">
						<a href="<%=path%>/bx/bxsh/cllist?pages=${pages + 1}">下一页</a>
					</c:when>
					<c:otherwise>
					下一页
				</c:otherwise>
