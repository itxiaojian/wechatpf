   ...
   if(isMob.test(s)||isPhone.test(s)){my=true}
	...
	function toSubmit(){
		...
		if(openId==null||openId==''){
			url="<%=path%>/wx/Bxspyjb/update";
		}else{
			url="<%=path%>/wx/Bxsq/update";
		}
		$.ajax({
			cache : true,
			type : "POST",
			...
			success : function(data) {
				if(data=='1'){
					...
					if(yhbh==null||yhbh==''){
						...
						location.href ="<%=path%>/wx/Bxspyjb/toBxsqlcjl?yhbh="+yhbh;
					}
				}else{
					alert("提交失败，请联系管理员。");
				}
...
<div style="display:none;">
	...
	<form class="center" id="new_entry" action="" accept-charset="UTF-8"
		...
		<input type="hidden" id="yhbh" name="yhbh" value="${yhbh }">
			...
			<div class="form-content">
				...
				<div class="form-fields-container">
								...
								<div class="controls">
									<select id="ly" name="ly">
										...
										<c:forEach var="data" items="${ly}" varStatus="obj">
											<option value="${data.zdz }" <c:if test="${map.ly==data.zdz }">selected="selected"</c:if>>${data.zdz}</option>
										</c:forEach>
									</select>
								</div>
					...
					<div class="field" data-api-code="field_14">
						<div class="form-group ">
							...
							<div class="field-content">
								...
								<div class="controls">
									<%-- <input type="text" value="${map.lh }" name="lh"  onkeyup="this.value=this.value.replace(/\D/g,'')" 
									onafterpaste="this.value=this.value.replace(/\D/g,'')"
										id="lh" maxlength="4"> --%>
										<input type="text" value="${map.lh}" name="lh" id="lh" />
								</div>
							...
							<div class="field-content">
								...
								<div class="controls">
									<select id="fw" name="fw">
										<option value="-1">请选择</option>
										<option value="2" <c:if test="${map.fw !='1' }" >selected="selected"</c:if>>维修</option>
									</select>
								</div>
					...
					<div class="field" data-api-code="field_16">
						<div class="form-group ">
							<label class="control-label field_title" onclick=""
								data-role="collapse_toggle" for="wxdl"> 维修大类 <span>（最少选1项）</span>
								<span style="color: red;">*</span>
							</label>
							<div class="field-content">

								<div class="controls">
									<select id="wxdl" name="wxdl" onblur="getWxxl(this.value);">
										<option value="-1">请选择</option>
										<c:forEach var="data" items="${wxdl}" varStatus="obj">
											<option value="${data.id }_${data.ztmc}" <c:if test="${map.wxdl==data.ztmc }">selected="selected"</c:if>>${data.ztmc}</option>
										</c:forEach>
									</select>
								</div>
							...
							<div class="field-content">
								...
								<div class="controls">
									<select id="wxxl" name="wxxl">
										<option value="-1">请选择</option>
										<option selected="selected" value="${map.sbztbh}_${map.sbztmc}">${map.sbztmc}</option>
									</select>
								</div>
					...
					<div class="field" data-api-code="field_18">
						<div class="form-group ">
							...
							<div class="field-content">
								...
								<div class="controls">
									<input type="text" value="${map.nr }" name="nr"
										id="nr">
								</div>
...
<!-- 										id="entry_field_19"> -->
					...
					<div class="field" data-api-code="field_20">
						<div class="form-group ">
							...
							<div class="field-content">
								...
								<div class="controls">
									<input type="tel" value="${map.lxhm }" name="lxhm"  onkeyup="this.value=this.value.replace(/\D/g,'')" 
									onafterpaste="this.value=this.value.replace(/\D/g,'')"
										id="lxhm" >
								</div>
		...
		<%-- 			<div class="field" data-api-code="field_21">
						<div class="form-group ">
							<label class="control-label field_title" onclick=""
								data-role="collapse_toggle" for="bz"> 其它说明 </label>
							<div class="field-content">

								<div class="controls">
									<input type="text" value="${map.bz }" name="bz"id="bz">
										<script type="text/plain" id="myEditor" style="width:216px;height:100px;align:left;">${map.bz }</script>
					    ...
					    <c:if test="${fn:length(fn:split(map.bz,';'))==3 }">
						<c:forEach var="item" varStatus="status" begin="0" end="${fn:length(fn:split(map.bz,';'))-1}">
 						   <div class="field-content uploadDiv${status.count}">
							<div id="add-product-image"  style="z-index: 1;" onclick="toAddImg(${status.count});">
									<img id="img${status.count}" alt="+" src="${fn:split(map.bz,';')[status.index]}"  align="center" style="height: 160px;width:160px; text-align: center;"/>
						    </div>
                            <div id="uploadDiv"  class="controls" style="position: absolute; top: 25px; left: 35px; width: 163px; height: 161px; overflow: hidden; z-index: 0;">
	        	               <input id="file${status.count}"  name="file" type="file" style="font-size: 999px; opacity: 0; position: absolute; top: 0px; left: 0px; width: 100%; height: 100%;" accept="image/*" capture="camera" onchange="selectFile(${status.count});">
	                        </div> 
	                         <input type='hidden' id='fileName${status.count}' name='bz${status.count}' value="${fn:split(map.bz,';')[status.index]}" />
							</div>
						</c:forEach>
						</c:if>
						<c:if test="${fn:length(fn:split(map.bz,';'))==2 }">
						<c:forEach var="item" varStatus="status" begin="0" end="${fn:length(fn:split(map.bz,';'))-1}">
 						   <div class="field-content uploadDiv${status.count}">
							<div id="add-product-image"  style="z-index: 1;" onclick="toAddImg(${status.count});">
									<img id="img${status.count}" alt="+" src="${fn:split(map.bz,';')[status.index]}"  align="center" style="height: 160px;width:160px; text-align: center;"/>
						...
						<div class="field-content uploadDiv3" style="display:none;">
							<div id="add-product-image"  style="z-index: 1;" onclick="toAddImg(3);">
									<img id="img3" alt="+" src=""  align="center" style="height: 160px;width:160px; text-align: center;"/>
						    </div>
                            <div id="uploadDiv"  class="controls" style="position: absolute; top: 25px; left: 35px; width: 163px; height: 161px; overflow: hidden; z-index: 0;">
						...
						<c:if test="${fn:length(fn:split(map.bz,';'))==1 && map.bz!=''}">
							<div class="field-content uploadDiv1">
							<div id="add-product-image"  style="z-index: 1;" onclick="toAddImg(1);">
									<img id="img1" alt="+" src="${map.bz}"  align="center" style="height: 160px;width:160px; text-align: center;"/>
						    </div>
	                        ...
	                        <input type='hidden' id='fileName1' name='bz1' value='${map.bz}'>
							</div>
							<div class="field-content uploadDiv2" style="display:none;">
							<div id="add-product-image"  style="z-index: 1;" onclick="toAddImg(2);">
									<img id="img2" alt="+" src=""  align="center" style="height: 160px;width:160px; text-align: center;"/>
						    </div>
                            <div id="uploadDiv"  class="controls" style="position: absolute; top: 25px; left: 35px; width: 163px; height: 161px; overflow: hidden; z-index: 0;">
						...
						<c:if test="${fn:length(fn:split(map.bz,';'))==1 && map.bz==''}">
                            ...
                            <div id="uploadDiv"  class="controls" style="position: absolute; top: 25px; left: 35px; width: 163px; height: 161px; overflow: hidden; z-index: 0;">
	        	               <input id="file1"  name="file" type="file" style="font-size: 999px; opacity: 0; position: absolute; top: 0px; left: 0px; width: 100%; height: 100%;" accept="image/*" capture="camera" onchange="selectFile(3);">
	                        </div> 
	                         <input type='hidden' id='fileName3' name='bz3' value=''>
							</div>
						</c:if>	
