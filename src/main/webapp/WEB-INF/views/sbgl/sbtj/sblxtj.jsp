...
.bordered tr:last-child td:last-child {
    ...
    function showECharts(){
        ...
        $.ajax({
			  type: 'POST',
			  url: '<%=path%>/sbgl/SbSbtj/getSblxtj',
			  data:{
				     id :lx,
					 stime:starttime,
					 etime:endtime,
...
<div style="display: none;">
		<ul class="tab-menu tab" id="tabMenuID_">
			<li class="tab-selected" title="设备类型统计"><a
				href="<%=path%>/sbgl/SbSbtj/toSblxtjPage" target="content"
				onfocus="this.blur()"><span>设备类型统计</span></a></li>
		</ul>
	</div>
	<div id="table"
		style="height: 40%; border: 0px solid #ccc; padding: 10px;">
		<div>
		<select class="chzn-select" data-placeholder="设备类型"
			style="width:20%;" tabindex="1">
			<option value=""></option>
			<option value="">全部</option>
			<c:forEach var="data" items="${listBm}" varStatus="status">
			<option value="${data.sbxh}">${data.sbxh}</option>
			</c:forEach>
		</select>
		...
		<table class="bordered">
			<thead id="thead">
				<tr>
					...
					<td height="20" style="width: 305px;">设备类型</td>
