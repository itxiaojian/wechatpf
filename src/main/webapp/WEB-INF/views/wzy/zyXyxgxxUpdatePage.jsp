				...
				<div class="input-group">
					...
					<select class="form-control input-sm" name="xwlx" value="" >
				    	<c:forEach items="${list}" var="list">
			    			<c:if test="${map.xwlx == list.zdz}"><option value="${list.zdz}" selected="selected">${list.zdmc}</option></c:if>
		        			<c:if test="${map.xwlx != list.zdz}"><option value="${list.zdz}">${list.zdmc}</option></c:if>
						</c:forEach>
					</select>
				</div>
				<br>
...
<script type="text/javascript">
	...
	var options = {  
			//   target: '#output',          //把服务器返回的内容放入id为output的元素中     
			   ...
			   url: url+'/wzy/ZyXyxgxx/updateXYXGXX',   //默认是form的action， 如果申明，则会覆盖  
