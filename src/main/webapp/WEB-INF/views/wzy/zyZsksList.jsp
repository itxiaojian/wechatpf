...
<body id="zhuti">
	<div class="phone">
		<div class="logo">
			<img src="<%=path%>/resources/img/wzy/logo.png" style="height:1.6rem;">
		</div>
			...
			<div class="NEW_main">
			  <div class="NEW_msg">
				<ul id="tbl1">
				 ...
				 <a href="<%=path%>/wzy/ZyXyxw/zyZsksDetail?id=${list.id}&zdbm=${zdbm}">
			        ...
			        <c:choose>
					 	<c:when test="${fn:length(list.xwzs) > 15}">
							<c:out value="${fn:substring(list.xwzs,0,15)}..." />
						</c:when>
						<c:otherwise>
							<c:out value="${list.xwzs}" />
						</c:otherwise>
					</c:choose>
			        </span> </span>
			        <%if(i<=4){i++;}else{i=1;}%>
		...
		<div class="SY_icon">
			<a href="<%=path%>/wzy/ZyXyxgxx/zyXyjjDetail?zdbm=yztj&openId=${openId}"><img src="<%=path%>/resources/img/wzy/FH.png"></a>
		</div>
	</div>
</body>
</html>
