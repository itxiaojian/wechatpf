...
<script type="text/javascript">
		...
		var zNodes =[
			{ id:'0', name:"全部文件",url:"${base}/sys/SysFile/list", target:"documentFrame",open:true},
			<#list folders?if_exists as doc>
			{ id:'${doc.id}', <#if doc.parentId?has_content>pId:"${doc.parentId}",</#if> name:"${(doc.name)!}",url:"${base}/sys/SysFile/list?Q_folderId_S_EQ=${doc.id}", target:"documentFrame",icon:"${base}/libs/images/icons/folder.gif",open:true},
			</#list>
			{ id:'-1', pId:'0',name:"未分类",url:"${base}/sys/SysFile/list?Q_folderId_S_NULL=null", target:"documentFrame",icon:"${base}/libs/images/icons/folder.gif",open:true},
		];
		...
		function selectAll() {
		 ...
		 function showContent() {
		        ...
		        if (treeNodeId == false || treeNodeId == "false") {
		            ...
		            if (treeNodeId == "-1") {
			           parent.documentFrame.location= "${base}/sys/SysFile/list?Q_folderId_S_NULL=null";
		            } else if (treeNodeId == "0"){
			           parent.documentFrame.location= "${base}/sys/SysFile/list";
		            } else {
			           parent.documentFrame.location= "${base}/sys/SysFile/list?Q_folderId_S_EQ="+treeNodeId;
		            }
		        }
		    }
