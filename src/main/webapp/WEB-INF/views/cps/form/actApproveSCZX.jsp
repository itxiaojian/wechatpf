...
<script type="text/javascript"
	src="/resources/js/jquery/plugins/jquery.form.js"></script>
<script type="text/javascript"
	src="/resources/js/cps/webSign/webSign.js"></script>
...
<body onload="showWebSign()">
	...
	<SCRIPT language=javaScript
		src="/resources/js/cps/webSign/webSignObject.js">
	...
	<div class="panel panel-success">
		...
		<table class="table table-bordered">
			...
			<tr>
				<th class="thbg">授信形式</th>
				<td>${mapBusi.creditNatureName}</td>
				<th class="thbg"></th>
				<td></td>
				<th class="thbg"></th>
				<td></td>
	...
	<!-- 审批意见表单填写 -->
	<div class="panel panel-success">
		<div id="opinionDiv" class="panel-heading"><font color="red">*</font> 填写审查意见:</div>
		<div class="panel-body">
			<form name="submitForm" id="optionForm" method="post"
				enctype="multipart/form-data">
				<input type="hidden" name="businessKey" value="${businessKey}" /> <input
					type="hidden" name="taskId" value="${taskId}" /> <input
					type="hidden" name="userCode" value="${userCode}" /> <input
					type="hidden" name="lineVariable" value="${lineVar}" /> <input
					type="hidden" name="value" id="valueInputId" />
				...
				<span style="color: red">* 请上传审查报告：</span> <input type="file"
					name="attachMentFile" id="attachMentFile" size="25"> <br>
				<div class="input-group">
					<span class="input-group-addon"><font color="red">*</font> 同意授信金额 ( 元 )</span>
					<input type="text" class="form-control" placeholder="请输入允许的授信金额"
						value="${mapBusi.creditSumW}" name="creditSumW"
						id="creditSumWInputId" />
				</div>
				<div class="input-group">
				...
				<div class="input-group">
					<span class="input-group-addon">授信条件是否已改变 </span> <input
						type="radio" name="hasChang" value="0" lang="1" checked="checked" />
					没有改变 <input type="radio" name="hasChang" value="1" /> 改变
				</div>
				<div align="center">
					<input type="button" class="btn btn-danger"
						name="seal_optionContent"
						onclick="WebSign_AddSeal('${userCode}');" value="盖章" /> <input
						type="button" class="btn btn-primary" name="write_optionContent"
						onClick="HandWritePop_onclick2('${userCode}')" value="手写" /> <input
						type="submit" class="btn btn-primary" onclick="agree();"
						value="批准" /> <input type="submit" class="btn btn-danger"
						onclick="disagree();" value="不批准" />
				</div>
...
<script type="text/javascript">
	...
	var options = {
		...
		url : '/cps/form/dealActSCZX', //默认是form的action， 如果申明，则会覆盖  
		//type: type,               //默认是form的method（get or post），如果申明，则会覆盖  
		dataType : 'json', //html(默认), xml, script, json...接受服务端返回的类型  
		//clearForm: true,          //成功提交后，清除所有表单元素的值  
		//resetForm: true,          //成功提交后，重置所有表单元素的值  
		timeout : 10000
	//限制请求的时间，当请求大于10秒后，跳出请求  
	};
	...
	function checkApproveData() {
		...
		var newCreditRate = $("#creditRateInputId").val();
		var newCreditYear = $("#creditYearInputId").val();
		var optinoContent = $("#optinoContentId").value();
		alert(optinoContent);
