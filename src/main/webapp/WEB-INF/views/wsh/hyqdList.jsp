			...
			<div class="NEW_main">
			  <div class="NEW_msg">
				<ul id="tbl1">
				 ...
				 <a href="<%=path%>/wxutil/meeting/lshyDetail?id=${list.id}&openId=${openId}">
			   			 <li><span class='new_msg01'>
			    	
			    	<c:choose>
					 	<c:when test="${fn:length(list.hybt) > 12}">
							<c:out value="${fn:substring(list.hybt,0,12)}..." />
						</c:when>
						<c:otherwise>
							<c:out value="${list.hybt}" />
						</c:otherwise>
					</c:choose>
			        <br><span class='new_msg02'>
			        <c:choose>
					 	<c:when test="${fn:length(list.hynr) > 15}">
							<c:out value="${fn:substring(list.hynr,0,15)}..." />
						</c:when>
						<c:otherwise>
							<c:out value="${list.hynr}" />
						</c:otherwise>
					</c:choose>
			        </span> </span>
			        <%if(i<=4){i++;}else{i=1;}%>
