...
<body id="zhuti" onload="ScrollDiv();">
	<div class="phone">
		<div class="logo">
			...
			<div class="NEW_main">
			  <div class="NEW_msg">
				<ul id="tbl1">
				 ...
				 <a href="<%=path%>/wsh/ShTpzl/shtpzlDetail?id=${list.id}&openId=${openId}">
			   			 <li><span class='new_msg01'>
			    	
			    	<c:choose>
					 	<c:when test="${fn:length(list.tpnr) > 12}">
							<c:out value="${fn:substring(list.tpnr,0,12)}..." />
						</c:when>
						<c:otherwise>
							<c:out value="${list.tpnr}" />
						</c:otherwise>
					</c:choose>
			        <br><span class='new_msg02'>
<%-- 			        <c:choose> --%>
<%-- 					 	<c:when test="${fn:length(list.ztbt) > 15}"> --%>
							<c:out value="${fn:substring(list.fbsj,0,19)}" />
<%-- 						</c:when> --%>
<%-- 						<c:otherwise> --%>
...
<!-- 	   </div> -->
		...
		<div class="SY_icon">
			<a href="<%=path %>/wsh/zy/zhuye?openId=${openId}"><img src="<%=path%>/resources/img/wzy/FH.png"></a>
		</div>
	</div>
</body>
