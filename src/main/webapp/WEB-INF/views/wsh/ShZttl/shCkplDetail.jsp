...
<body>
 ...
 <div style="float:right;margin-right:100px;margin-top:10px;">
			 <c:choose>
				<c:when test="${pages > 1}">
				   <a href="<%=path%>/wsh/ShZttl/shCkplDetail?id=${id }&pages=${pages - 1}">上一页</a>
			    </c:when>
			    <c:otherwise>
				                     上一页
			    </c:otherwise>
			...
			<c:choose>
				<c:when test="${pages <count}">
				   <a href="<%=path%>/wsh/ShZttl/shCkplDetail?id=${id }&pages=${pages +1}">下一页</a>
				</c:when>
				<c:otherwise>
				                    下一页
				</c:otherwise>
 ...
 <div class="zhanti_title" style="margin-top:10px;">
        <img src="<%=path%>/resources/img/wzy/wypl.png" style="width:3%;"> 编辑专题最后结果:
 </div>
 <br>
 
 
<!--  <form id="myForm" class="wwx_f_a" method="post" enctype="multipart/form-data" style="display: inline;"> -->
<%--         <input type="hidden" name="projectName" id="projectName" value="${pageContext.request.contextPath}"/> --%>
		<input type="hidden" name="id" id="id" value="${id}"/>
 <textarea rows="3" cols="30" name="ztzhjg" id="ztzhjg"
   style="width:70%;height:200px;font-size:15px;margin-left:30px;border: #2991e6 1px solid;"></textarea> <br>
 
 <div  style="margin-top:10px;margin-left:35%;">
 		 <input type="submit"  onclick="agree('${id}');" value="保存" />
 		 <a href="javascript:;history.back()"><input type="button" value="关闭" /></a>
 </div>
<!--  </form> -->
...
function agree(){
	...
	function agree(id) {   //回复
		if ($("#ztzhjg").val() == "") {
			return;
		}
		var ztzhjg = $("#ztzhjg").val();
		$(document).ready(
				function() {
					$.ajax({
						type : "POST",
						url : "savezhjg",
						data : { id:id,ztzhjg:ztzhjg},
