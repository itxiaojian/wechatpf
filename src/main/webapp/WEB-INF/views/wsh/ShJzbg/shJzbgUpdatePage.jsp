		 ...
		 <script language="Javascript" src="<%=path%>/resources/js/xzxx/Calendar.js" type="text/javascript"></script>
	      <div id="meizzCalendarLayer" name="meizzCalendarLayer" style="position: absolute; z-index: 9999; width: 144px; height: 193px; display: none">
			...
			<form name="submitForm" id="optionForm" method="post"
				...
				<input type="hidden" name="jznr" id="jznr"/>
				<input type="hidden" name="id" id="id" value="${map.id}"/>
				<div class="input-group">
					<span class="input-group-addon"><font color="red">*</font> 讲座名称</span>
					<input type="text" class="form-control" placeholder="请输入讲座名称"
						value="${map.jzmc}" name="jzmc"
						id="jzmc" />
				</div>
				<br>
				<div class="input-group">
					<span class="input-group-addon"><font color="red">*</font> 讲座标题</span>
					<input type="text" class="form-control" placeholder="请输入讲座标题"
						value="${map.jzbt}" name="jzbt"
						id="jzbt" />
				</div>
				<br>
				<div class="input-group">
					<span class="input-group-addon"><font color="red">*</font> 备注</span>
					<input type="text" class="form-control" placeholder="请输入备注"
						value="${map.bz}" name="bz"
						id="bz" />
				</div>
				<br>
				<!--style给定宽度可以影响编辑器的最终宽度-->
				<script type="text/plain" id="myEditor" style="width:99%;height:10%;">
						${map.jznr}
				</script>
				<br>
				<div align="center">
...
<script type="text/javascript">
	...
	var options = {  
			//   target: '#output',          //把服务器返回的内容放入id为output的元素中     
			   ...
			   url: url+'/wsh/ShJzbg/updateJzbg',   //默认是form的action， 如果申明，则会覆盖  
	...
	function agree() {
		...
		document.getElementById("jznr").value=UM.getEditor('myEditor').getContent();
		if(returnFlag) {
