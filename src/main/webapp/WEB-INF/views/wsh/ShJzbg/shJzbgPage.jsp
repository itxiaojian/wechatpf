	...
	<div class="main">
		...
		<div class="bottom" style="border:0px;">
			...
			<div class="title_name" style="width: 100%;margin-top:-1%">
				<span class="title_nametext">&nbsp;&nbsp;讲座报告</span> 
			...
			<div class="New_list">
				...
				<tr>
				           <a style="margin-left:-12px;" href="<%=path%>/wsh/ShJzbg/shJzbgpageDetail?id=${list.id}&openId=${openId}">
								<c:choose>  
									<c:when test="${fn:length(list.jzbt) > 15}">  
     									<c:out value="${fn:substring(list.jzbt,0,15)}..." />  
									</c:when>  
									<c:otherwise>  
     									<c:out value="${list.jzbt}" />  
									</c:otherwise>  
							    </c:choose>
							</a>
							<span style="float:right;">${list.jzsj }</span>
				</tr>
				</table>
				</c:forEach>
