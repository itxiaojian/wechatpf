...
<html><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
	...
	<script type="text/javascript">
		...
		function loadMore(page,lx,openId){
			i=page;
			i++;
			location.href ="<%=path%>/wsh/ShSwzl/toSkzlList?lx="+lx+"&pages="+i+"&openId="+openId;
		}
		function query(){
			var keyWord = $("#bt").val();
		...
		function deleteWp(id,openId){
			if(confirm("您确定要删除吗？")){
				location.href ="<%=path%>/wsh/ShSwzl/deleteskzl?id="+id+"&openId="+openId;
			}
		}
...
.fanhui{margin-top:0;float:right;margin-right:4%;width:10%;}
...
<div class="top_01">
<form action="<%=path%>/wsh/ShSwzl/toSkzlList" id="myForm" method="post">  
    	<span class="span_hz" style="width:62%; padding-left:5%">
        	拾卡招领:&nbsp;&nbsp;
              <input class="span_input"  name="bt" id="bt"  
...
<%-- 			</c:if> --%>
<%-- 			<c:if test="${lx=='3'}"> --%>
<!-- 				<span class="action-bar__tie__norm ng-binding ng-scope" ng-switch-when="1" ng-bind="actionBar.viewList[0].name">拾卡</span> -->
<%-- 			</c:if> --%>
			<c:if test="${lx==''}">
...
<!-- 			        &#39;active&#39;: helpList.isLost -->
<!-- 			    }"> -->
<%-- 				<a href="<%=path%>/wsh/ShSwzl/toSkzlList?lx=3&openId=${openId}"> --%>
<%-- 					<span class="-list01-item__main" <c:if test="${lx=='3' }">style="background: #f06b6b none repeat scroll 0 0;"</c:if> > --%>
<!-- <!-- 						<i class="-list01-item__icon fa fa-heart"></i> --> 
<%-- 						<img alt="" src="<%=path%>/resources/js/wsh/getwp.png" width="50px" height="50px"> --%>
<!-- 					</span> -->
<!-- 					<span class="-list01-item__intro"> -->
...
<!-- 			</li> -->
			<li class="-list01-item">
				<a href="<%=path%>/wsh/ShSwzl/toSkzlAdd?openId=${openId}">
...
<!-- 						<i class="-list01-item__icon fa fa-search"></i> -->
	...
	<div class="-bd scroll-y" id="panelBodyWrapper-5" scroll-full="changedPos" style="padding: 160px 0 0;">
		...
		<ul class="-list02">
			...
			<a href="<%=path%>/wsh/ShSwzl/toSkzlDetail?id=${data.id }&openId=${openId}" style="text-decoration:none;color:black;">
