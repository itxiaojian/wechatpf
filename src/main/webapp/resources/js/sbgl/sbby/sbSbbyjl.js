var DICT_GRID_STORE_URL = '/sbgl/SbSbbyjl/getSbbyjlList';
...
SbwxjhForm = Ext.extend(Ext.ux.Form, {
    ...
    constructor: function() {
		this.wxzt = this.createCombo('<span style="color:red">*</span>保养状态', 'zdz', 'zdmc', 'wxzt', '95%', PROJECT_NAME+'/sbgl/SbSbbyjh/getZt');
		this.wxzt.store.load();
		this.wxzt.allowBlank = false;
		...
		this.bypf = new Ext.form.NumberField({
            fieldLabel: '保养评分',
            name: 'bypf',
            allowBlank: true,
            allowNegative :false,
            maxLength:2,
            maxLengthText:'长度超过不能2', 
            anchor: '95%',
            regex: /^\d+(\.\d+)?$/,
            blankText: '该选项为必填项,请输入内容...'
        });
		this.gzms = this.createTextArea('保养描述','gzms','100%','97.5%', null);
     ...
     updateClick: function() {       //修改
         if(this.getForm().isValid()) {
         	...
         	this.getForm().submit({
                 ...
                 method: 'POST',
                 params:{
                 	id:record[0].get('id'),wxzt:this.wxzt.getValue(),gzms:this.gzms.getValue(),
                 	bypf:this.bypf.getValue(),kssj:this.kssj.getValue().format('Y-m-d'),wcsj:this.wcsj.getValue().format('Y-m-d')
                 },
                 success: function(form, action) { 
...
LookForm = Ext.extend(Ext.ux.Form, {
    ...
    constructor: function() {
		...
		this.wxdh = this.createTextField('<font color="red">*</font>保养单号','wxdh','95%','',null,100,'长度超过不能100');
		...
		this.bypf.readOnly = true;
		this.gzms = this.createTextArea('<span style="color:red">*</span>保养描述','gzms','100%','95%', null);
...
DictTypeGrid = Ext.extend(UxGrid, {
    ...
    constructor: function(height, width){
    	...
    	this.store = new Ext.data.Store({          //Grid Store
            ...
            reader: new Ext.data.JsonReader({totalProperty: 'total', root:'rows'},
            [
            		{name:'id'}, {name:'sbbh'}, {name:'sbmc'}, {name:'sbxh'},{name:'sybm'},{name:'wxdh'},
		            {name:'wxsqr'}, {name:'gzms'},{name:'wxdw'}, {name:'wxfzr'}, {name:'wxzt'},{name:'wxjb'},
		            {name:'kssj'}, {name:'wcsj'}, {name:'bypf'}, {name:'jb'}
            ])
        });
...
//                   {text:'计划完成',iconCls: 'edit',handler:this.onWcClick,scope:this},'-',
               	   ...
               	   {xtype:'label',text:'保养级别：'},{xtype:'combo',id:'wxjb',
	  				   editable: false,
	  				   store: new Ext.data.Store({
	  			         proxy: new Ext.data.HttpProxy({url: PROJECT_NAME+'/sbgl/SbSbbyjl/getByjb', method: 'POST'}),
        ...
        DictTypeGrid.superclass.constructor.call(this, {
            ...
            cm: new Ext.grid.ColumnModel([new Ext.grid.RowNumberer(),
	            ...
	            {header: '保养单号', width: 100, dataIndex: 'wxdh', sortable: true,hidden: false},
	            {header: '保养级别', width: 100, dataIndex: 'jb', sortable: true,hidden: false},
	            {header: '保养申请人', width: 100, dataIndex: 'wxsqr', sortable: true,hidden: false},
	            {header: '保养单位', width: 100, dataIndex: 'wxdw', sortable: true,hidden: false},
	            {header: '保养人', width: 100, dataIndex: 'wxfzr', sortable: true,hidden: false},
	            {header: '保养状态',dataIndex:'wxzt',width:100,sortable:true,hidden:false,
	            ...
	            {header: '保养评分', width: 100, dataIndex: 'bypf', sortable: true,hidden: false},
	            {header: '保养描述',dataIndex:'gzms',width:100,sortable:true,hidden:false}
            ]),
            tbar: this.vtbar,
            bbar: this.vbbar,
            ds: this.store
