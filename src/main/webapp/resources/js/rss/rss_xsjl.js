...
var html=""; 
var bgcolor="";
var url = path+"/rss/rssXsjlDetail";
		$.ajax({
			url : url,
			data : {
				url_rss : url_rss,
			...
			success : function(data) {
				...
				$("item", rst.message).each(
						function(index, element) {
							...
							html += "<li><a title='"+$(this).find("title").text().replace(tem,"").substring(5)+"' href='"
							+ path+"/rss/main_xsjl?url="+f [0].replace("<![CDATA[","")+"&title="+$(this).find("title").text().replace(tem,"").substring(5)
							+"&time="+$(this).find("pubDate").text()+"&author="+$(this).find("author").text()
							+ "'>"
							+ FormatContent($(this).find("title").text().replace(tem,""),16).substring(5)
							+"</a><span class='New_list_time'>"
							+ formatDate($(this).find("pubDate").text()).substring(0,11)
							+"</span></li>";
						});
