var USER_GRID_STORE_URL = 'getLxxgxxList';
var PAGESIZE=20;
    ...
    constructor: function(height, width){
    	this.store = new Ext.data.Store({          //Grid Store
            ...
            reader: new Ext.data.JsonReader({totalProperty: 'total', root:'rows'},[
            		{name:'id'},{name:'type'},{name:'title'},{name:'content'},{name:'fbtime'},{name:'ly'},{name:'author'}
...
//                {header:'发布人编号',dataIndex:'author',width:80,sortable: true, hidden:true},
//                {header:'部门编号',dataIndex:'ly',width:80,sortable: true, hidden:true},
//            	{header:'新闻标题',dataIndex:'xwbt',width:200,sortable: true},
            	{header:'信息标题',dataIndex:'title',width:200,sortable: true, 
                    renderer:function(value, cellmeta, record){
                    	var id = record.data.id;
                    	return '<a target="_blank" href="lxXgxxDetail?id=' + id + '">' + value + '</a>';
                    }
            	},
            	{header:'发布人',dataIndex:'author',width:200,sortable: true},
            	{header:'来源',dataIndex:'ly',width:200,sortable: true},
            	{header:'发布时间',dataIndex:'fbtime',width:200,sortable: true},
            	{header:'信息类型',dataIndex:'type',width:260,sortable: true, 
            		renderer:function(value){
	                    if(value == '0') {
	                        return "<span style='color:blue;'>办理指南</span>";
	                    }else if(value == '1') {
	                        return "<span style='color:red;'>离校公告</span>";
	                    }else{
	                    	return value;
	                    }
            		}
    ...
    addByEditor: function() {//用网页编辑器编辑新闻
    	var url = "lxXgxxListPage";   	
    	html = '<iframe scrolling="auto" frameborder="0" width="100%" height="100%" src="' + url + '"></iframe>';    		
    	ACT_DEAL_WINDOW = new ActDealWindow();
    	ACT_DEAL_WINDOW.setTitle("发布离校相关信息");
    ...
    onModifyClick: function() {
    	var records=this.getSelectionModel().getSelections();
   		if(records.length > 0) {
   			if(records.length == 1){
   				vrecord = records[0];
   				var url = "lxXgxxUpdatePage?id="+vrecord.get('id');   	
   		    	html = '<iframe scrolling="auto" frameborder="0" width="100%" height="100%" src="' + url + '"></iframe>';    		
   		    	ACT_DEAL_WINDOW = new ActDealWindow();
   		    	ACT_DEAL_WINDOW.setTitle("修改离校相关信息");
   		    	ACT_DEAL_WINDOW.html = html;
   		    	ACT_DEAL_WINDOW.show();
   			}else{
