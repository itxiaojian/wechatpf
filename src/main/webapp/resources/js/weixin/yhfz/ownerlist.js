    ...
    constructor: function() {
     ...
     updateFormClick: function() {       //修改
         if(this.getForm().isValid()) {
         	...
         	this.getForm().submit({
                 waitMsg: '正在提交数据...',
                 url: PROJECT_NAME+'/wxutil/yhfz/update', 
                 method: 'POST',
                 success: function(form, action) { 
    ...
    constructor: function(height, width){
        ...
        MeetMenberGrid.superclass.constructor.call(this, {
            ...
            cm: new Ext.grid.ColumnModel([new Ext.grid.RowNumberer(),
                ...
                {header:'部门名称',dataIndex:'bmmc',width:260,sortable: true},
                {header:'人员姓名',dataIndex:'xm',width:100,sortable: true}
            ]),
            bbar: this.vbbar,
            ds: this.store
        });
    ...
    constructor: function(height, width){
        ...
        DictTypeGrid.superclass.constructor.call(this, {
            ...
            cm: new Ext.grid.ColumnModel([new Ext.grid.RowNumberer(),
	            ...
	            {header: '分组名称', width: 200, dataIndex: 'fzmc', sortable: true,hidden: false},
	            {header:'发起人',dataIndex:'cjr',width:100,sortable:true,hidden:true},
	            {header:'添加时间',dataIndex:'tjsj',width:180,sortable:true,hidden:false},
	            {header:'分组说明',dataIndex:'bz',width:750,sortable:true,hidden:false,title:true}
            ]),
            tbar: this.vtbar,
            bbar: this.vbbar,
            ds: this.store
    	...
    	var records=this.getSelectionModel().getSelections();
   		if(records.length > 0) {
   			if(records.length == 1){
   		    	...
   		    	winForm.fzmc.setValue(vrecord.data.fzmc);
				winForm.fzsm.setValue(vrecord.data.bz);
    ...
    onDeleteClick: function() {
    	...
    	var valueStr=[];
   		if(records.length>0) {
	    	...
	    	Ext.Msg.confirm("提醒信息", "确定要删除这  " + records.length + " 条分组吗?",function(btn){
				if (btn == 'yes') {
			       	Ext.Ajax.request({
