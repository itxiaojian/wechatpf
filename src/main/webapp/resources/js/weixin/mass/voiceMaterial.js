...
var ENTITY_URL_UPLOAD = "/wxauth/materialmgr/uploadVoice";
...
MyUploadForm = Ext.extend(Ext.ux.Form,{
	constructor: function(){
		this.tishi = new Ext.form.DisplayField({
			value:'<span  style="color:red;font-weight:bold;">语音大小2M，播放长度不超过60s，支持AMR/MP3格式</span>'
		});
	...
	onUploadClick: function(){
		if(this.getForm().isValid()){
        	...
        	if(attachmentType != "mp3" && attachmentType != "amr"){
        		Ext.MessageBox.alert("系统提示:", BLANKSTR + "语音类型为AMR\MP3" + BLANKSTR);
        		return;
        	}
...
UnteckAttachmentGrid = Ext.extend(UxGrid,{
	constructor: function(height,width){
		...
		this.vtbar = new Ext.Toolbar({
			items: [
					'-',{xtype:'button',text:'上传语音',iconCls:'db-icn-upload',handler:this.onUploadClick,scope:this},
					'-',{xtype:'label',text:'素材名称'},{xtype:'textfield',id:'fileName'},
					//'-',{xtype:'label',text:'素材类型'},query_filetype,
			       '-',{xtype:'button',text:'查询',iconCls:'query',handler:function(){
							...
							//var sclx = query_filetype.getValue();
							var sclx = "2";
			       			unteckAttachmentGrid.store.baseParams = {scmc:scmc,sclx:sclx};
							
	    	   				//unteckAttachmentGrid.store.baseParams= params;
	    	   				unteckAttachmentGrid.store.load({params:{start:0,limit:PAGESIZE}});
	    	   			}
	       			},
		...
		this.vcm = new Ext.grid.ColumnModel([new Ext.grid.RowNumberer(),
		            ...
		            {header:'素材名称',dataIndex:'filename',width:200,sortable:true},
