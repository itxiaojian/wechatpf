var USER_GRID_STORE_URL = 'getTxltjList';
var PAGESIZE=20;
...
ConstructionForm = Ext.extend(Ext.ux.Form, {
    ...
    constructor: function() {
    	...
    	this.yhxm = this.createTextField('用户姓名:', 'yhxm', '85%');
        this.dhhm = this.createTextField('电话号码:', 'dhhm', '85%');
        this.bz = this.createTextField('备注:', 'bz', '85%');
        this.yhxm.allowBlank = true;
        this.dhhm.allowBlank = true;
    ...
    constructor: function(height, width){
    	this.store = new Ext.data.Store({          //Grid Store
            ...
            reader: new Ext.data.JsonReader({totalProperty: 'total', root:'rows'},[
                           {name:'id'},{name:'yhxm'},{name:'dhhm'},{name:'bz'}
            ])
        });
      ...
      /*      	'-',{xtype:'textfield',id:'code',width: 150,emptyText:''},
      		    ...
      		    '-',{xtype:'button',text:'下载EXCEL模板',iconCls:'excel',handler:function(){
	  		    		Ext.Msg.confirm('系统提示','确定要下载该模板吗?',function(btn){
	 		        	    if(btn=="yes"){ 
	 		        	    	window.open(PROJECT_NAME+"/resources/js/wfw/Txltj.xls");
	 		        	   }
	 		        	 });
	 		    	 },scope:this},
	 		    '-',{xtype:'button',text:'导入EXCEL数据',iconCls:'excel',handler:this.onUploadClick,scope:this},
                '-',{xtype:'button',text:'下载通讯录',iconCls:'excel',handler:function(){
	   		    		Ext.Msg.confirm('系统提示','下载通讯录?',function(btn){
			        	    if(btn=="yes"){ 		        	    	
			        	    	var fileName = "通讯录";
			        	    	/*var code = Ext.getCmp('code').getValue();*/
			        	    	var url = PROJECT_NAME + "/wfw/ZsTxltj/export?fileName="+fileName;
			        	    	url = encodeURI(url);
			        	    	url = encodeURI(url);
			        	    	window.open(url);
			        	   }
...
//        });    	
        ...
        ConstructionGrid.superclass.constructor.call(this, {
            ...
            cm: new Ext.grid.ColumnModel([new Ext.grid.RowNumberer(),
                ...
                {header:'用户姓名',dataIndex:'yhxm',width:150,sortable: true},
                {header:'电话号码',dataIndex:'dhhm',width:150,sortable: true},
