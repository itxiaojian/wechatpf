...
Ext.ux.Portal.DropZone = function(a, b) {
	this.portal = a;
...
Ext
		.extend(
				...
				Ext.dd.DropTarget,
				{
					...
					notifyOver : function(v, t, w) {
						var f = t.getXY(), a = this.portal, n = v.proxy;
						if (!this.grid) {
						...
						if (a.fireEvent("validatedrop", g) !== false
								&& a.fireEvent("beforedragover", g) !== false) {
							n.getProxy().setWidth("auto");
							if (q) {
								n.moveProxy(q.el.dom.parentNode, k ? q.el.dom
										: null);
							} else {
								n.moveProxy(u.el.dom, null);
							}
							this.lastPos = {
								c : u,
								col : d,
					...
					notifyDrop : function(k, g, f) {
						delete this.grid;
						if (!this.lastPos) {
							return;
						}
						var i = this.lastPos.c, b = this.lastPos.col, j = this.lastPos.p;
						var a = this.createEvent(k, g, f, b, i, j !== false ? j
								: i.items.getCount());
						if (this.portal.fireEvent("validatedrop", a) !== false
								&& this.portal.fireEvent("beforedrop", a) !== false) {
							k.proxy.getProxy().remove();
							k.panel.el.dom.parentNode
									.removeChild(k.panel.el.dom);
							if (j !== false) {
								if (i == k.panel.ownerCt
										&& (i.items.items.indexOf(k.panel) <= j)) {
									j++;
								}
								i.insert(j, k.panel);
							} else {
