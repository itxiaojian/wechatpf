...
ST.ux.ViewGrid = Ext.extend(Ext.Viewport, {
	...
	createGrid: function() {
    	...
    	if(this.displayTopButton) {
	    	/*this.grid.getTopToolbar().insertButton(index++,'-');
	    	this.grid.getTopToolbar().insertButton(index++,new Ext.Button({text:"添加",iconCls: 'add', id:'addEntity'}));
	    	this.grid.getTopToolbar().insertButton(index++,new Ext.Button({text:"更新",iconCls: 'edit', id:'editEntity'}));
	    	this.grid.getTopToolbar().insertButton(index++,new Ext.Button({text:"删除",iconCls: 'delete', id:'delEntity'}));*/
    ...
    queryData: function() {
		Ext.apply(this.store.lastOptions.baseParams, this.queryForm.getForm().getFieldValues());
		this.grid.store.reload({params:{start:0,limit:this.grid.getBottomToolbar().pageSize}});
		if(this.east != null) {
    ...
    buildItems: function(flag) {
        ...
        for (var i = 0; i < this.girdColumns.length; i++) {
            ...
            if(col.emptyText !='') col['emptyText']=col.emptyText;
            items.push(col);
        }
