...
var orgPanel,appContextmenu,orgTreePanel= new Ext.tree.TreePanel({
            ...
            dataUrl : PROJECT_NAME+'/sys/SysZzjg/orgOrMebTree',
            ...
            tbar :[{
                ...
                text : '刷新',
                handler : function() {
                    orgTreePanel.loader.dataUrl = PROJECT_NAME+'/sys/SysZzjg/orgOrMebTree';
	...
	onBeforeLoad : function(node, e) {
		...
		if(isparentId == true){
			pid = areaIdStr;
		this.loader.dataUrl = PROJECT_NAME+"/sys/SysZzjg/findAttacheArea?id="+pid+"&isparentId=true"; //定义子节点的Loader
		isparentId = false;
		}else{
			if(pid==null){
				pid=areaIdStr;
			}
			this.loader.dataUrl = PROJECT_NAME+"/sys/SysZzjg/findAttacheArea?id="+pid; //定义子节点的Loader
		}
	}
...
createOrganizationFun = function (data,pData) {
	if(pData==null){p_id="0";}else{p_id=pData.id;}
        ...
        items:[
            {
                xtype:'form',
                plain:true,
                url:PROJECT_NAME+'/sys/SysZzjg/add',
                labelWidth:60,
                baseCls:"x-plain",
        ...
        items:[
            {
                xtype:'form',
                plain:true,
                url:PROJECT_NAME+'/sys/SysZzjg/edit',
                labelWidth:60,
                baseCls:"x-plain",
