var USER_GRID_STORE_URL = 'getTpzlList';
var PAGESIZE=20;
    ...
    constructor: function(height, width){
    	this.store = new Ext.data.Store({          //Grid Store
            ...
            reader: new Ext.data.JsonReader({totalProperty: 'total', root:'rows'},[
            		{name:'id'},{name:'tpnr'},{name:'fbsj'},{name:'jssj'},{name:'fqr'}
            		,{name:'zdxx'},{name:'zt'}
            ])
        });
...
//        });    	
        ...
        ConstructionGrid.superclass.constructor.call(this, {
            ...
            cm: new Ext.grid.ColumnModel([new Ext.grid.RowNumberer(),
            	...
            	{header:'投票内容',dataIndex:'tpnr',width:200,sortable: true 
//                    renderer:function(value, cellmeta, record){
//                    	var id = record.data.id;
//                    	return '<a target="_blank" href="zyXyxwDetail?id=' + id + '">' + value + '</a>';
//                    }
            	},
            	{header:'发布人',dataIndex:'fqr',width:150,sortable: true},
            	{header:'发布时间',dataIndex:'fbsj',width:150,sortable: true},
            	{header:'结束时间',dataIndex:'jssj',width:150,sortable: true},
            	{header:'最多选项',dataIndex:'zdxx',width:150,sortable: true},
            	...
            	{header: '操作', width: 150, align:"center",sortable: true,hidden: false,
					renderer:function(value, cellmeta, record){
						   ...
						   var id = record.data.id;
						   return '<a href="shTpzlXx?id='+ id +'" style="text-decoration:none;">添加选项</a>'
						   +"&nbsp;&nbsp;&nbsp;" +
       			   		'<a href="tpzlckxx?id='+ id +'"  style="color:red;cursor:pointer;text-decoration:none;">查看选项</a>';
						}
				}
            ]),
            tbar: this.vtbar,
    ...
    addByEditor: function() {//用网页编辑器编辑新闻
    	var url = "shTpzlListPage";   	
    	html = '<iframe scrolling="auto" frameborder="0" width="100%" height="100%" src="' + url + '"></iframe>';    		
    	ACT_DEAL_WINDOW = new ActDealWindow();
    	ACT_DEAL_WINDOW.setTitle("发布投票内容");
...
//			return false;}
   		if(records.length > 0) {
   			if(records.length == 1){
   				var url = "shTpzlUpdatePage?id="+vrecord.get('id');   	
   		    	html = '<iframe scrolling="auto" frameborder="0" width="100%" height="100%" src="' + url + '"></iframe>';    		
   		    	ACT_DEAL_WINDOW = new ActDealWindow();
   		    	ACT_DEAL_WINDOW.setTitle("修改投票内容");
   		    	ACT_DEAL_WINDOW.html = html;
   		    	ACT_DEAL_WINDOW.show();
	       		}else{
