var USER_GRID_STORE_URL = 'getSkList';
var PAGESIZE=20;
...
ConstructionForm = Ext.extend(Ext.ux.Form, {
    ...
    constructor: function() {
    	...
    	this.bt = this.createTextField('标题:', 'bt', '85%');
        this.yktkh = this.createTextField('学工号:', 'yktkh', '85%');
        this.dd = this.createTextField('地点:', 'dd', '85%');
        this.fbrxm = this.createTextField('联系人:', 'fbrxm', '85%');
        this.xwzs = this.createTextField('详情:', 'xwzs', '85%');
        this.bt.allowBlank = false;
        this.yktkh.allowBlank = false;
        this.dd.allowBlank = false;
        this.fbrxm.allowBlank = false;
        this.xwzs.allowBlank = false;
    ...
    constructor: function(height, width){
    	this.store = new Ext.data.Store({          //Grid Store
            ...
            reader: new Ext.data.JsonReader({totalProperty: 'total', root:'rows'},[
                           {name:'id'},{name:'lx'},{name:'bt'},{name:'xwzs'},{name:'fqsj'},{name:'jssj'},
                           {name:'fbr'},{name:'fbrxm'},{name:'xwzt'},{name:'dd'},{name:'yktkh'},{name:'bz'}
            ])
        });
...
//      			    }},
      		    '-',{xtype:'button',text:'下载EXCEL模板',iconCls:'excel',handler:function(){
	  		    		Ext.Msg.confirm('系统提示','确定要下载该模板吗?',function(btn){
	 		        	    if(btn=="yes"){ 
	 		        	    	window.open(PROJECT_NAME+"/resources/js/wsh/skxx.xls");
	 		        	   }
	 		        	 });
	 		    	 },scope:this},
 		       '-',{xtype:'button',text:'导入EXCEL数据',iconCls:'excel',handler:this.onUploadClick,scope:this},
 		       '-',{xtype:'button',text:'发送消息提醒',iconCls:'folder_go',handler:this.onSendClick,scope:this}
...
//        });    	
        ...
        ConstructionGrid.superclass.constructor.call(this, {
            ...
            cm: new Ext.grid.ColumnModel([new Ext.grid.RowNumberer(),
                ...
                {header:'标题',dataIndex:'bt',width:150,sortable: true},
                {header:'学工号',dataIndex:'yktkh',width:150,sortable: true},
            	{header:'地点',dataIndex:'dd',width:150,sortable: true},
            	...
            	*/
                {header:'联系人',dataIndex:'fbrxm',width:100,sortable: true},
            	{header:'详情',dataIndex:'xwzs',width:150,sortable: true},
            ]),
            tbar: this.vtbar,
            bbar: this.vbbar,
            ds: this.store,
    ...
    onSendClick: function() {
    	...
    	var valueStr=[];
   		if(records.length>0) {
	    	...
	    	Ext.Msg.confirm("提醒信息", "确定发送这 " + records.length + " 条信息吗",function(btn){
				if (btn == 'yes') {
			       	Ext.Ajax.request({
			       		url: 'sendsktx', 
				       	   method : 'POST', 
