var USER_GRID_STORE_URL = 'getJzbgList';
var PAGESIZE=20;
    ...
    constructor: function(height, width){
    	this.store = new Ext.data.Store({          //Grid Store
            ...
            reader: new Ext.data.JsonReader({totalProperty: 'total', root:'rows'},[
            		{name:'id'},{name:'jzmc'},{name:'jzbt'},{name:'jznr'},{name:'jzsj'},{name:'fbr'}
            		,{name:'bz'},{name:'zt'}
            ])
        });
...
//        });    	
        ...
        ConstructionGrid.superclass.constructor.call(this, {
            ...
            cm: new Ext.grid.ColumnModel([new Ext.grid.RowNumberer(),
            	...
            	{header:'讲座标题',dataIndex:'jzbt',width:150,sortable: true, 
                    renderer:function(value, cellmeta, record){
                    	var id = record.data.id;
                    	return '<a target="_blank" href="shJzbgDetail?id=' + id + '">' + value + '</a>';
                    }
            	},
            	{header:'专题名称',dataIndex:'jzmc',width:150,sortable: true},
            	{header:'发布人',dataIndex:'fbr',width:150,sortable: true},
            	{header:'发布时间',dataIndex:'jzsj',width:150,sortable: true},
            	...
            	{header: '操作', width: 150, align:"center",sortable: true,hidden: false,
					renderer:function(value, cellmeta, record){
						   ...
						   return  '<a href="shCkplDetail?id=' + id + '"  style="text-decoration:none;">查看评论</a>'+
						   "&nbsp&nbsp&nbsp"+
					   		  "<a href='javascript:;' onclick='constructionGrid.close()' style='text-decoration:none;'>"+
					   		  "<span style='width: 30px;cursor: pointer;'>关闭</span></a>";
...
//				if (btn == 'yes') {
			       	Ext.Ajax.request({
			       		url: 'close', 
				       	   ...
				       	   params: { ids: valueStr},
			               success: function(form, action) {
				               Ext.MessageBox.alert("系统提示:", BLANKSTR + "关闭成功!" + BLANKSTR);
				               constructionGrid.store.reload();
			               },
			               failure: function(form, action) {
			            	   Ext.MessageBox.alert("系统提示:", BLANKSTR + "关闭失败!" + BLANKSTR);
			               }
				       	});					
//				}
//	    	});	
    ...
    addByEditor: function() {//用网页编辑器编辑新闻
    	var url = "shJzbgListPage";   	
    	html = '<iframe scrolling="auto" frameborder="0" width="100%" height="100%" src="' + url + '"></iframe>';    		
    	ACT_DEAL_WINDOW = new ActDealWindow();
    	ACT_DEAL_WINDOW.setTitle("发布讲座");
...
//			return false;}
   		if(records.length > 0) {
   			if(records.length == 1){
   				var url = "shJzbgUpdatePage?id="+vrecord.get('id');   	
   		    	html = '<iframe scrolling="auto" frameborder="0" width="100%" height="100%" src="' + url + '"></iframe>';    		
   		    	ACT_DEAL_WINDOW = new ActDealWindow();
   		    	ACT_DEAL_WINDOW.setTitle("修改讲座");
   		    	ACT_DEAL_WINDOW.html = html;
   		    	ACT_DEAL_WINDOW.show();
	       		}else{
