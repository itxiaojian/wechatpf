BizQQWPA
		.define(
				"wpa.SelectPanel",
				"lang.browser,util.Style,util.className,util.events,util.offset,util.css,util.proxy,lang.extend",
				function(require) {
					var Style = require("Style"), className = require("className"), events = require("events"), offset = require("offset"), browser = require("browser"), css = require("css"), proxy = require("proxy"), extend = require("extend");
					var NODE_TYPE_ELEMENT = 1;
					var doc = document;
					var findById = function(parent, id) {
						var append = function(host, tail) {
							for (var i = 0, len = tail.length; i !== len; host
									.push(tail[i++])) {
							}
							return host;
						}, children = append([], parent.childNodes), len = children.length, pos = 0, curNode, ret = null;
						while (len > pos) {
					...
					SelectPanel.prototype = {
						render : function() {
							...
							events.addEvent(findById(frame,
									"WPA3-SELECT-PANEL-CLOSE"), "click",
									function() {
										panel.remove();
										opts.onClose && opts.onClose()
									});
							events.addEvent(findById(frame,
									"WPA3-SELECT-PANEL-AIO-CHAT"), "click",
									function() {
										panel.remove();
										opts.onAIOChat && opts.onAIOChat()
									});
							events.addEvent(findById(frame,
									"WPA3-SELECT-PANEL-ANONY-CHAT"), "click",
									function() {
										panel.remove();
						...
						renderModal : function() {
							var container = this.container, width = css(
									...
									container, "overflow");
							var modalLayer = doc.createElement("div"), styles = {
								position : "fixed",
								top : 0,
								left : 0,
								zIndex : 2147483647,
		...
		.define(
				...
				function(require) {
					...
					var inDom = function(node, fn) {
						...
						}, parent = node.parentNode, nextSibling = node.nextSibling, div = document
								.createElement("div"), returnValue;
					...
					var css = function(node, style, value) {
						...
						if (!value) {
							...
							if (typeof style !== "object") {
								new TypeError(
										"Arg style should be string or object")
							}
							cssText = [];
						...
						cssText = cssText.replace(/([A-Z])/g, "-$1")
								.toLowerCase();
		...
		.define(
				...
				function() {
					return document.documentElement.contains ? function(node,
							context) {
						var adown = node.nodeType === 9 ? node.documentElement
								: node, bup = context && context.parentNode;
						return node === bup
								|| !!(bup && bup.nodeType === 1
										&& adown.contains && adown
										.contains(bup));
					}
							: document.documentElement.compareDocumentPosition ? function(
								...
								return context
										&& !!(node
												.compareDocumentPosition(context) & 16)
							}
