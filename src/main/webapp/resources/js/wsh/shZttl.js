var USER_GRID_STORE_URL = 'getZttlList';
var PAGESIZE=20;
    ...
    constructor: function(height, width){
    	this.store = new Ext.data.Store({          //Grid Store
            ...
            reader: new Ext.data.JsonReader({totalProperty: 'total', root:'rows'},[
            		{name:'id'},{name:'ztmc'},{name:'ztbt'},{name:'ztnr'},{name:'fbsj'},{name:'jhjssj'},{name:'fbr'}
            		,{name:'ztzhjg'},{name:'zt'}
            ])
        });
...
//        });    	
        ...
        ConstructionGrid.superclass.constructor.call(this, {
            ...
            cm: new Ext.grid.ColumnModel([new Ext.grid.RowNumberer(),
            	...
            	{header:'专题标题',dataIndex:'ztbt',width:150,sortable: true, 
                    renderer:function(value, cellmeta, record){
                    	var id = record.data.id;
                    	return '<a target="_blank" href="shZttlDetail?id=' + id + '">' + value + '</a>';
                    }
            	},
            	{header:'发布人',dataIndex:'fbr',width:100,sortable: true},
            	{header:'发布时间',dataIndex:'fbsj',width:150,sortable: true},
            	{header:'计划结束时间',dataIndex:'jhjssj',width:150,sortable: true},
            	{header:'专题名称',dataIndex:'ztmc',width:150,sortable: true},
            	{header:'专题最后结果',dataIndex:'ztzhjg',width:150,sortable: true},
    ...
    addByEditor: function() {//用网页编辑器编辑新闻
    	var url = "shZttlListPage";   	
    	html = '<iframe scrolling="auto" frameborder="0" width="100%" height="100%" src="' + url + '"></iframe>';    		
    	ACT_DEAL_WINDOW = new ActDealWindow();
    	ACT_DEAL_WINDOW.setTitle("发布专题");
...
//			return false;}
   		if(records.length > 0) {
   			if(records.length == 1){
   				var url = "shZttlUpdatePage?id="+vrecord.get('id');   	
   		    	html = '<iframe scrolling="auto" frameborder="0" width="100%" height="100%" src="' + url + '"></iframe>';    		
   		    	ACT_DEAL_WINDOW = new ActDealWindow();
   		    	ACT_DEAL_WINDOW.setTitle("修改专题");
   		    	ACT_DEAL_WINDOW.html = html;
   		    	ACT_DEAL_WINDOW.show();
	       		}else{
