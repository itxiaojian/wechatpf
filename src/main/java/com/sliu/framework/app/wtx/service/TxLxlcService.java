...
@Service
public class TxLxlcService extends BaseBO<TxLxlcTxjl> {
	
	@Autowired
	private TxLxlcDao dao;
	...
	public void sendWagesNews(){
		...
		List<Map<String,Object>> list = dao.getLxlcjl(year);
		if(list.size()==0){
		...
		String url = "";
		List<Map<String,Object>> listlj = dao.getDict("lxlj");
		if(listlj.size()>0){
		...
		if(listWage.size()>0){
		 ...
		 if(listWage.size()>0){
				for(Map<String,Object> map:listWage){
					TxLxlcTxjl txjl =new TxLxlcTxjl();
					String toUser = map.get("wxh").toString();
					txjl.setTacheid(map.get("tacheid").toString());
					txjl.setStuid(map.get("stuid").toString());
					txjl.setStuname(map.get("stuname").toString());
					txjl.setBlzt(map.get("blzt").toString());
					txjl.setBltime(map.get("bltime").toString());
					txjl.setWxh(toUser);
					dao.save(txjl);
				}
		    }
		 dao.deleteLxlcjl(lastyear);
	   }
}
